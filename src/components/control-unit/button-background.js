...
const ButtonBacground = (props) => {
  ...
  // useEffect(() => {
  //   fetch(`https://api.unsplash.com/photos/random?orientation=landscape&per_page=1&query=clody&client_id=${KEY_API_BACKGROUND}`)
  //     .then((response) => response.json())
  //     .then((data) => {
  //       changeBackground(data.urls.full)
  //       document.querySelector('#root').style.backgroundImage = `url(${data.urls.full})`
  //     })
  // }, [background]);
